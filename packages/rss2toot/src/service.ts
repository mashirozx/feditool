...
export class Service {
  ...
  public async run() {
    const { handler, logger, redis } = this
    ...
    for (const status of statuses) {
      ...
      const mediaAttachments = await this.handleMediaAttachments(
   ...
   * Handle medias
   */
  private handleMediaAttachments(formattedStatusContent: Formatter) {
    const downloader = new MediaDownloader(this.logger)
    return downloader.handleFormattedStatusContent(
      formattedStatusContent,
